...
`.agents/workflows/repo-analyzer.md` - even though the uninstall path removes them.
"""
...
def test_antigravity_project_install_writes_rules_and_workflows(tmp_path):
    ...
    skill = tmp_path / ".agents" / "skills" / "repo-analyzer" / "SKILL.md"
    rules = tmp_path / ".agents" / "rules" / "graphify.md"
    workflow = tmp_path / ".agents" / "workflows" / "repo-analyzer.md"
...
def test_antigravity_workflow_names_no_skill_path(tmp_path):
    ...
    point at ~/.gemini/config/skills/repo-analyzer/SKILL.md, which it never writes.
    """
    m._project_install("antigravity", tmp_path)
    body = (tmp_path / ".agents" / "workflows" / "repo-analyzer.md").read_text(encoding="utf-8")
    ...
    assert "repo-analyzer skill" in body, "workflow should still point at the skill by name"
...
def test_antigravity_global_install_workflow_names_no_skill_path(tmp_path, monkeypatch):
    ...
    assert (
        tmp_path / "home" / ".gemini" / "config" / "skills" / "repo-analyzer" / "SKILL.md"
    ).exists()
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".agents" / "workflows" / "repo-analyzer.md").exists()
    assert not (tmp_path / ".agents" / "skills" / "repo-analyzer" / "SKILL.md").exists()
