...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/repo-analyzer --update` — this function only signals
    ...
    if flag.exists():
        ...
        print("[graphify check-update] Run `/repo-analyzer --update` to apply semantic re-extraction.")
    return True
...
def _notify_only(watch_path: Path) -> None:
    ...
    print("[graphify watch] Run `/repo-analyzer --update` in Claude Code to update the graph.")
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /repo-analyzer --update (LLM extraction required).
    ...
    print(f"[graphify watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /repo-analyzer --update.")
